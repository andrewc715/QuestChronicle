...
Quest Chronicle v1.11.2 enforces a maximum of 500 physical lines per runtime Lua file. The public subsystem namespaces remain stable while implementation helpers and shared private state live behind internal namespace tables.
...
`Core/ZoneStyle/Zone` owns the v1.11.2 Zone Context Snapshot, ordered registries, evidence ledger, compatibility compiler, canonical style evidence, selected-look affinity, and on-demand diagnostics.
...
`Compatibility.lua` compiles the snapshot back into the exact v1.10.0 context shape. The existing Zone Native scoring and generation modules remain authoritative in v1.11.2 and do not read canonical style evidence.

`Affinity.lua` observes only already selected visible pieces. Affinity format 2 adds explicit `VALUE`, `MISSING`, and `NOT_APPLICABLE` component states while preserving the format-1 arithmetic. It combines the shared descriptor language with Zone palette, material, finish, motif, culture, magic, provenance, and avoid evidence. It does not enumerate candidate pools, alter generation scores, or consume random values.

`ExportEncoding.lua` owns the reversible `DIAGNOSTIC_ESCAPE_V1` transport representation used for arbitrary dynamic values. `DebugExport.lua` assembles Zone debug export format 2 for `/qc zone debug export`. It reads the immutable context, complete evidence ledger, selected-look affinity, generation-mode registry, and latest Zone Native report summary. It does not persist an export, invoke generation, enumerate candidates, alter the preview, or consume random values. `UI/DebugReport.lua` reuses the existing copy dialog used by Traveler tuning exports.
...
`Core/Diagnostics/ReportCompaction.lua` owns the ordered, policy-aware compaction pipeline. It removes duplicated outfit rows, display-only profile copies, per-component Zone policy calculations, and persisted per-piece Zone affinity detail before lower-value timing payloads. The authoritative `zoneFoundation.anchorPolicy` summary, selected identities, support-profile ancestry, Phase D result, warnings, and headline performance remain intact.

`Core/Diagnostics/History.lua` persists diagnostic format 1 at `QuestChronicleDB.debug`. The store retains at most ten reports, rejects malformed or incompatible entries, caps each report at 20 KB, caps approximate history at 200 KB, and prunes oldest entries first. Valid oversized reports are compacted before insertion. A report that still cannot fit prints a visible warning and emits `DIAGNOSTIC_REPORT_REJECTED`; it may not disappear silently. Clearing the store is isolated from Chronicle history, wardrobe caches, concepts, and Courier data.
