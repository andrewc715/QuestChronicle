#!/usr/bin/env python3
"""Verify the v1.11.3 Zone anchor-policy authority transfer and parity boundary."""
...
checks = {
    "clean numeric v1.11.3 metadata": version == "1.11.3" and "## Version: 1.11.3" in toc,
    ...
    "debug export format advances to 3": "Zone.DEBUG_EXPORT_FORMAT = 3" in exporter and "Zone Anchor Policy" in exporter,
...
if failed:
    print("FAIL: v1.11.3 Zone anchor-policy guard failed:")
    for name in failed: print("  -", name)
    raise SystemExit(1)
print(f"PASS: v1.11.3 Zone anchor-policy verification: {len(checks)} checks")
