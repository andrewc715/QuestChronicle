...
required_compaction = [
    "function P.CompactReportToLimit",
    "report.outfit.slots = nil",
    "profile.activeAnchors = nil",
    "report.performance.phaseStats = nil",
    "foundation.affinity.pieces = nil",
    "component.anchorPolicy = nil",
]
for marker in required_compaction:
    assert marker in compaction, f"missing diagnostic compaction marker: {marker}"
...
for marker in required_history:
    assert marker in history, f"missing diagnostic persistence marker: {marker}"
...
print("PASS diagnostic report compaction wiring, Zone duplicate trimming, and visible rejection handling")
