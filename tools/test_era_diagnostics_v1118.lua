...
local era={operations=444,siblingCompletions=37,localOperations=390,apiOperations=54,apiAdmissions=54,apiHeadroomDeferrals=7,freshOnlyDeferrals=0,phantomDeferrals=0,sourceCacheCompletions=31,fragmentCacheCompletions=22,freshSliceDeferrals=0,deferredReturns=7,sameSliceDeferredRetries=0,synchronousProgressGuardTrips=0,executionMode="GENERATION_COOPERATIVE",fragmentCacheHits=22,fragmentCacheBuilds=15,
...
local report,msg=D.AddReport({
 formatVersion=D.FORMAT_VERSION,id="QCDBG-ERA-1",sequence=1,timestamp=1,timestampText="2026-08-06",version="1.11.10",
...
for _,key in ipairs({"operations","siblingCompletions","localOperations","apiOperations","apiAdmissions","apiHeadroomDeferrals","freshOnlyDeferrals","phantomDeferrals","sourceCacheCompletions","fragmentCacheCompletions","deferredReturns","sameSliceDeferredRetries","synchronousProgressGuardTrips","executionMode","fragmentCacheHits","largestSubphase","largestSubphaseMs"}) do
...
for _,expected in ipairs({"Era evidence scheduling: 444 operations","37 siblings","390 local","54 API","22 fragment hits","Era admission: 54 API admissions","7 headroom deferrals","0 fresh-only","0 phantom","31 source-cache completions","22 fragment-cache completions","Era execution boundary: GENERATION_COOPERATIVE","0 same-slice retries","0 synchronous guard trips","Largest era subphase: Era set-list acquisition 2.75 ms"}) do
...
local emergency=D.AddReport({formatVersion=D.FORMAT_VERSION,id="QCDBG-ERA-2",sequence=2,timestamp=2,version="1.11.10",action="GENERATE_OUTFIT",mode="ZONE_NATIVE",result="COMPLETED",success=true,
...
print(string.format("PASS v1.11.10 era diagnostics survive adaptive tier %s and emergency persistence",tostring(report.compaction.tierLabel)))
