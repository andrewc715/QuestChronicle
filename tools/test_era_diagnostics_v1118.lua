...
local era={operations=444,siblingCompletions=37,freshSliceDeferrals=19,fragmentCacheHits=22,fragmentCacheBuilds=15,
...
local report,msg=D.AddReport({
 formatVersion=D.FORMAT_VERSION,id="QCDBG-ERA-1",sequence=1,timestamp=1,timestampText="2026-08-06",version="1.11.8",
...
for _,key in ipairs({"operations","siblingCompletions","freshSliceDeferrals","fragmentCacheHits","largestSubphase","largestSubphaseMs"}) do
...
for _,expected in ipairs({"Era evidence scheduling: 444 operations","37 siblings","19 fresh-slice deferrals","22 fragment hits","Largest era subphase: Era set-list acquisition 2.75 ms"}) do
...
local emergency=D.AddReport({formatVersion=D.FORMAT_VERSION,id="QCDBG-ERA-2",sequence=2,timestamp=2,version="1.11.8",action="GENERATE_OUTFIT",mode="ZONE_NATIVE",result="COMPLETED",success=true,
...
print(string.format("PASS v1.11.8 era diagnostics survive adaptive tier %s and emergency persistence",tostring(report.compaction.tierLabel)))
