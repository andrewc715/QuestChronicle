#!/usr/bin/env python3
"""Verify the complete v1.11.3 shared-generation extraction contract."""
...
if version != "1.11.3" or "## Version: 1.11.3" not in toc:
    fail("package metadata is not the clean numeric v1.11.3 release")
...
if "EXTRACTION_BRIDGE" in "\n".join((ROOT / r).read_text(encoding="utf-8") for r in required):
    fail("completed v1.11.3 runtime still contains EXTRACTION_BRIDGE")

print("PASS: v1.11.3 shared framework, Traveler policy, legacy adapters, API routing, diagnostics, and numeric version contract are present.")
