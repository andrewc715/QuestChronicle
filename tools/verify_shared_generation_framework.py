#!/usr/bin/env python3
"""Verify the complete v1.11.11 shared-generation extraction contract."""
...
if version != "1.11.11" or "## Version: 1.11.11" not in toc:
    fail("package metadata is not the clean numeric v1.11.11 release")
...
if "EXTRACTION_BRIDGE" in "\n".join((ROOT / r).read_text(encoding="utf-8") for r in required):
    fail("completed v1.11.11 runtime still contains EXTRACTION_BRIDGE")

print("PASS: v1.11.11 shared framework, Traveler policy, legacy adapters, API routing, diagnostics, and numeric version contract are present.")
