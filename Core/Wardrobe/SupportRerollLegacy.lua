...
local function CurrentDecision(job, profile, node, slotKey)
...
local function BuildRoot(job, profile, targetSlot)
    ...
    local node = { selected = {}, decisions = {}, budget = P.CreateSupportBudget(job.draft, active), totalScore = 0 }
    for _, slotKey in ipairs(active) do if slotKey ~= targetSlot then CurrentDecision(job, profile, node, slotKey) end end
    return node, active
end
...
local function Choose(decisions)
    ...
    local minimum, total = shortlist[#shortlist].score, 0
...
local function BuildDiagnostics(profile, node, chosen, active, chosenRank, shortlistSize, poolSize)
    ...
    return {
        ...
        poolSizes = { [chosen and chosen.slotKey or ""] = poolSize or 0 },
        expansions = { [chosen and chosen.slotKey or ""] = poolSize or 0 },
        retained = { [chosen and chosen.slotKey or ""] = shortlistSize or 0 },
        deduplicated = 0, budgetRejections = 0, emptySlots = 0, decisions = decisions, activeSlots = active,
    }
end
