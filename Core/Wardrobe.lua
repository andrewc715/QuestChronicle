...
local function IsMeleeWeaponFamily(familyKey)
    return familyKey == "ONE_HAND" or familyKey == "TWO_HAND"
end

local function IsPhysicalMeleeWeaponPair(topology)
    return topology
        and topology.hasWeaponOffHand == true
        and IsMeleeWeaponFamily(topology.mainHandKind)
        and IsMeleeWeaponFamily(topology.offHandKind)
end

local function GetTopologyMainFamilyGate(topology)
    ...
    if topology.mainHandKind == "RANGED" then
        allowed.RANGED = true
    elseif IsMeleeWeaponFamily(topology.mainHandKind) then
...
local function GetWeaponRouteCacheKey(topology, mainOutfitSlot, offOutfitSlot)
    ...
    if GetSpecialization and GetSpecializationInfo then
        ...
        specID = specIndex and SafeCall(GetSpecializationInfo, specIndex) or nil
    end
    return table.concat({
...
local function DecorateOptionCandidates(options, ownerSlot, borrowed)
    local result = {}
...
local function MergeSecondaryOptionCandidates(secondaryOptions, primaryOptions, secondarySlot, primarySlot)
    local merged = {}
    local seen = {}
...
local function BuildFamilyOptionParts(outfitSlot, options, allowedFamilies, model, targetLabel)
    local partsByFamily = { ONE_HAND = {}, TWO_HAND = {}, RANGED = {} }
    for _, optionInfo in ipairs(options or {}) do
        local queryResult = QueryWeaponOptionSubtypes(outfitSlot, optionInfo)
        local recognized = 0
        local accepted = 0
        for _, familyKey in ipairs(MAIN_ROUTE_FAMILIES) do
            ...
            if familyCount > 0 then
                ...
                if allowedFamilies[familyKey] then
                    local subtypes = CopySubtypePermissionMap(queryResult.subtypes, familyKey, false)
...
local function CreateSingleWeaponRoute(model, mainPart, familyKey)
    local optionInfo = mainPart.optionInfo
    local route = {
        id = MakeWeaponRouteID(mainPart.optionOwnerSlot, optionInfo, familyKey, "SINGLE"),
        familyKey = familyKey,
        ...
        primarySubtypes = CopySubtypePermissionMap(mainPart.subtypes, familyKey, false),
        secondarySubtypes = {},
...
local function CreatePairedWeaponRoute(model, mainPart, offPart, familyKey, inheritedSecondary)
    ...
    local route = {
        ...
        primarySubtypes = CopySubtypePermissionMap(mainPart.subtypes, familyKey, false),
        secondarySubtypes = CopySubtypePermissionMap(
    ...
    route.available = RouteHasCachedSubtype(route, "PRIMARY") and RouteHasCachedSubtype(route, "SECONDARY")
    table.insert(model.routes, route)
    table.insert(model.routesByFamily[familyKey], route)
end
...
function Wardrobe.GetWeaponAppearanceRoutes(forceRefresh)
    ...
    local physicalWeaponPair = IsPhysicalMeleeWeaponPair(topology)
    ...
    local allowedFamilies = GetTopologyMainFamilyGate(topology)
    model.allowedTopologyFamilies = allowedFamilies

    local rawMainOptions = GetWeaponOptionCandidatesForOutfitSlot(mainOutfitSlot)
    local mainOptions = DecorateOptionCandidates(rawMainOptions, mainOutfitSlot, false)
    local mainParts = BuildFamilyOptionParts(mainOutfitSlot, mainOptions, allowedFamilies, model, "MAIN")

    if physicalWeaponPair then
        local rawOffOptions = GetWeaponOptionCandidatesForOutfitSlot(offOutfitSlot)
        local offOptions = MergeSecondaryOptionCandidates(rawOffOptions, mainOptions, offOutfitSlot, mainOutfitSlot)
        local offParts = BuildFamilyOptionParts(offOutfitSlot, offOptions, allowedFamilies, model, "SECONDARY")
            ...
            for _, mainPart in ipairs(primaryParts) do
                ...
                for _, offPart in ipairs(secondaryParts) do
                    if offPart.optionInfo.weaponOption == mainPart.optionInfo.weaponOption then
                        CreatePairedWeaponRoute(model, mainPart, offPart, familyKey, false)
                        created = true
                    end
                end
                ...
                if not created and nativeLinkedPair and CountMapEntries(mainPart.subtypes) > 0 then
                    CreatePairedWeaponRoute(model, mainPart, nil, familyKey, true)
                end
            end
        end
    else
        for _, familyKey in ipairs(MAIN_ROUTE_FAMILIES) do
            for _, mainPart in ipairs(mainParts[familyKey] or {}) do
                CreateSingleWeaponRoute(model, mainPart, familyKey)
            end
        end
    end
    ...
    if topology.offHandKind == "OFF_HAND" and offOutfitSlot ~= nil then
        local rawOffOptions = GetWeaponOptionCandidatesForOutfitSlot(offOutfitSlot)
        local offOptions = DecorateOptionCandidates(rawOffOptions, offOutfitSlot, false)
    ...
    if #model.routes == 0 and topology.mainItem then
        ...
        if MAIN_ROUTE_FAMILY_SET[physicalFamily] and allowedFamilies[physicalFamily] then
            ...
            for _, subtypeKey in ipairs(GetWeaponSubtypeKeysForFamily(physicalFamily)) do
                ...
                if physicalWeaponPair and SafeCall(C_TransmogCollection and C_TransmogCollection.IsCategoryValidForItem, categoryID, topology.offItem) == true then
                    secondarySubtypes[subtypeKey] = { key = subtypeKey, familyKey = physicalFamily, categoryID = categoryID }
                end
            end
            if CountMapEntries(primarySubtypes) > 0
                and (not physicalWeaponPair or CountMapEntries(secondarySubtypes) > 0)
            then
                local route = {
